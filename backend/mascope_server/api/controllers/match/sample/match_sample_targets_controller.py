...
async def get_match_sample_ions(
    ...
    Steps:
    1. Fetch the sample item data from the database including the batch ID and name.
    ...
    6. Sort the merged data based on specified criteria.
    7. Paginate the sorted data based on page and limit parameters.
    ...
    match_data = {item["target_ion_id"]: item for item in match_ions["data"]}
    match_sample_ions = []
    ...
    # Sorting and pagination logic
    match_sample_ions_sorted = sorted(
        match_sample_ions,
        key=lambda x: (
    ...
    match_sample_ions_paginated = match_sample_ions_sorted[
        page * limit : (page + 1) * limit
    ]

    return {
        "results": len(match_sample_ions_sorted),
        "message": f"Successfully retrieved target ion matches for sample '{sample_item_name}'.",
        "data": match_sample_ions_paginated,
    }
...
async def get_match_sample_isotopes(
    ...
    optionally ordered by match score or category.
    ...
    7. Convert the list to a DataFrame and apply filtering parameters.
    8. Sort the filtered DataFrame based on provided order criteria.
    9. Paginate the sorted data.
    ...
    :type target_collection_id: Optional[str], optional
    :param order: Column name to sort by, defaults to None (typically 'match_score' or 'match_category').
    :type order: Optional[str], optional
    :param page: Page number for pagination, defaults to 0.
    :type page: int, optional
    :param limit: Number of items per page, defaults to 10000.
    ...
    for isotope in target_isotopes["data"]:
        ...
        # Add sample instrument to each record
    ...
    match_sample_isotopes_df_filtered = apply_filter_params(match_sample_isotopes_df)

    # Sorting and pagination logic
    match_sample_isotopes_df_sorted = match_sample_isotopes_df_filtered.sort_values(
        ["match_category", "match_score"], ascending=[False, (order != "desc")]
    )
    match_sample_isotopes_df_paginated = match_sample_isotopes_df_sorted.iloc[
        page * limit : (page + 1) * limit
    ]

    return {
        "results": len(match_sample_isotopes_df_sorted),
        "message": f"Successfully retrieved target isotopes matches for sample '{sample_item_name}'.",
        "data": match_sample_isotopes_df_paginated.to_dict(orient="records"),
    }
