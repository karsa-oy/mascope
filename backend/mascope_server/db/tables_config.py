...
def get_table_configs(version=None) -> dict:
    ...
    version is not provided or a configuration for the given version does not exist, it defaults to the
    current table configuration.

    :param version: The version number of the database schema for which to fetch configurations.
    :type version: int, optional
    :return: Dictionary containing the table configuration, which includes details such as column specifications,
             foreign key constraints, and the SQL command to create the table.
    :rtype: dict
    """
    if version:
        ...
        try:
            ...
            runtime.logger.info(f"Using schema configuration for version {version}.")
            return config
        except KeyError:
        ...
        runtime.logger.info("No version specified, using current schema configuration.")
...
# Current version of the schema configuration.
table_configs = {
    "workspace": {
        "columns": {
