...
class Field():
    def __init__(self, kecu, node_id, label, parent_frame, monitorable, settable, toggleable, row=None, column=None):
        self.kecu = kecu
...
class DiField(Field):
    def __init__(self, kecu, node_id, channel, label, parent_frame, **kwargs):
        ...
        self.checkbox.configure(state=tk.DISABLED)
        if self.kecu:
            node = self.kecu.nodes.get(node_id, None)
            if node:
                device = node._device
...
class App(tk.Tk):
    ...
    def build(self):
        ...
        # MION:Common
        self.fields[NodeId.MION_MFC5_MAIN].append(MfcField(self.kecu, NodeId.MION_MFC5_MAIN, "Main flow", mion_common_frame, row=0, column=0))
        VoltageField(self.kecu, NodeId.ALL_NODES, "Accelerator voltage", mion_common_frame, row=1, column=0)
        # /
        # MION:IS1
        self.fields[NodeId.MION_MFC2_SRC1_CRR].append(MfcField(self.kecu, NodeId.MION_MFC2_SRC1_CRR, "Carrier flow", mion_is1_frame, row=0, column=0))
        self.fields[NodeId.MION_MFC1_SRC1_EXH].append(MfcField(self.kecu, NodeId.MION_MFC1_SRC1_EXH, "Exhaust flow", mion_is1_frame, row=1, column=0))
        VoltageField(self.kecu, NodeId.ALL_NODES, "Deflector voltage", mion_is1_frame, row=2, column=0)
        # /
        # MION:IS2
        self.fields[NodeId.MION_MFC4_SRC2_CRR].append(MfcField(self.kecu, NodeId.MION_MFC4_SRC2_CRR, "Carrier flow", mion_is2_frame, row=0, column=0))
        self.fields[NodeId.MION_MFC3_SRC2_EXH].append(MfcField(self.kecu, NodeId.MION_MFC3_SRC2_EXH, "Exhaust flow", parent_frame=mion_is2_frame, row=1, column=0))
        VoltageField(self.kecu, NodeId.ALL_NODES, "Deflector voltage", mion_is2_frame, row=2, column=0)
        # /
        # MION:X-ray
        self.fields[NodeId.MION_DIO].append(DoField(self.kecu, NodeId.MION_DIO, 4, "Emission", mion_xray_frame, row=0, column=0))
        self.fields[NodeId.MION_DIO].append(DiField(self.kecu, NodeId.MION_DIO, 3, "Active", mion_xray_frame, row=1, column=0))
        self.fields[NodeId.MION_DIO].append(DiField(self.kecu, NodeId.MION_DIO, 1, "Enabled", mion_xray_frame, row=2, column=0))
        self.fields[NodeId.MION_DIO].append(DiField(self.kecu, NodeId.MION_DIO, 2, "Interlock", mion_xray_frame, row=3, column=0))
        self.fields[NodeId.MION_DIO].append(DiField(self.kecu, NodeId.MION_DIO, 0, "Alert", mion_xray_frame, row=4, column=0))
        # /
        # MION:Ion filter
        self.fields[NodeId.MION_DIO].append(DoField(self.kecu, NodeId.MION_DIO, 5, "Power", mion_if_frame, row=0, column=0))   
        self.fields[NodeId.MION_AI].append(MonitorField(self.kecu, NodeId.MION_AI, 4, "HV+", mion_if_frame, row=1, column=0))
        self.fields[NodeId.MION_AI].append(MonitorField(self.kecu, NodeId.MION_AI, 5, "HV-", mion_if_frame, row=2, column=0))
        # /
        # MION:Sensors
        self.fields[NodeId.MION_AI].append(MonitorField(self.kecu, NodeId.MION_AI, 0, "Humidity", mion_sensor_frame, row=0, column=0))
        self.fields[NodeId.MION_AI].append(MonitorField(self.kecu, NodeId.MION_AI, 1, "Temperature", mion_sensor_frame, row=1, column=0))
        self.fields[NodeId.MION_AI].append(MonitorField(self.kecu, NodeId.MION_AI, 2, "Pressure", mion_sensor_frame, row=2, column=0))
        # /
        # //
        ...
        # SH:Flows
        self.fields[NodeId.SH_MFC5_RGT].append(MfcField(self.kecu, NodeId.SH_MFC5_RGT, "Reagent flow", sh_mfc_frame, row=0, column=0))
        self.fields[NodeId.SH_MFC3_SMP].append(MfcField(self.kecu, NodeId.SH_MFC3_SMP, "Sample flow", sh_mfc_frame, row=1, column=0))
        self.fields[NodeId.SH_MFC2_EXH].append(MfcField(self.kecu, NodeId.SH_MFC2_EXH, "Exhaust flow", sh_mfc_frame, row=2, column=0))
        self.fields[NodeId.SH_MFC4_SHT1].append(MfcField(self.kecu, NodeId.SH_MFC4_SHT1, "Sheath 1 flow", sh_mfc_frame, row=3, column=0))
        self.fields[NodeId.SH_MFC1_SHT2].append(MfcField(self.kecu, NodeId.SH_MFC1_SHT2, "Sheath 2 flow", sh_mfc_frame, row=4, column=0))
        # /
        # //

        # # Calibrator
        self.fields[NodeId.CALIB_MFC].append(MfcField(self.kecu, NodeId.CALIB_MFC, "Carrier flow", cal_frame, row=0, column=0))
        # # //

        # # Flushplate
        self.fields[NodeId.FLSHP_MFC].append(MfcField(self.kecu, NodeId.FLSHP_MFC, "Counter flow", fp_frame, row=0, column=0))
        # # //
    ...
    def update_fields(self):
        for node, node_fields in self.fields.items():
            for field in node_fields:
                field.enable()
                if self.kecu and node in self.kecu.nodes:
                    field.enable()
                else:
                    field.disable()
