...
async def item_compute(
    ...
    with conn:
        ...
        [match_interferences_exist] = pd.read_sql(
            f"""
            SELECT
                CASE
        ...
        match_interference_df.to_sql(
            "match_interference", conn, if_exists="append", index=False
        )
...
async def match_item_compute(sid, sample_item):
    ...
    try:
        ...
        task = sio.start_background_task(item_compute, sample_item)
