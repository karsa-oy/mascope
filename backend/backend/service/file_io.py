...
from backend.lib.file import append_instrument_log, read_instrument_log, zarr_sdk
...
class FileIoNamespace(BaseClientNamespace):
    ...
    async def on_instrument_log_request(self, data):
        ...
        await self.emit_client_notification("instrument_log", log, room=client_room)
...
def run():
    ...
    if args["ns"] == "/":
        print(
            "file_io must be in a private namespace. "
            + "Please restart the service with --ns option."
        )
        return

    client = FileIoClient(args["url"], args["port"], (args["ns"], FileIoNamespace))
