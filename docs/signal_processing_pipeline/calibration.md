...
Mass-to-charge ratio ($m/z$) calibration is a critical procedure required to ensure high mass accuracy by aligning observed detector signals with theoretical isotopic standards.
...
The TOF architecture maps ion arrival times to mass coordinates using a multi-point regression that requires a minimum of 3 calibration points. 
