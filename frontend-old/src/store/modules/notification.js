...
export default {
  ...
  actions: {
    ...
    async onRematchBatchesFinished({ commit, state, dispatch }, data) {
      ...
      if (data.samples_compute_failed && data.samples_compute_failed.length > 0) {
    ...
    async onMatchItemUpdateComputeStarted({ commit }, data) {
      ...
      commit('SET_PROGRESS_MESSAGE', `Computing matches for "${data.sample_item_name}"`)
      commit('SET_PROGRESS_PERCENTAGE', 0)
    },
    ...
    async onMatchItemUpdateComputeFinished({ commit }, data) {
      commit('SET_PROGRESS_MESSAGE', `Computing matches for "${data.sample_item_name}" is finished`)
      commit('SET_PROGRESS_PERCENTAGE', 100)
      setTimeout(() => {
        // TODO_configuration move 500 animation delay to config file and 3000 closing notification time
    ...
    async onMatchItemUpdateComputeFailed({ dispatch, commit }, data) {
      commit('SET_PROGRESS_MESSAGE', `Computing matches failed for "${data.sample_item_name}"`)
      commit('SET_PROGRESS_PERCENTAGE', 100)
      commit('SET_COMPUTE_ERROR', true)
      setTimeout(() => {
        // Show warning notification after 3 seconds with info about failed to compute matches samples
    ...
    async onCalibrationStarted({ commit }, data) {
      ...
      commit('SET_PROGRESS_MESSAGE', `Calibration process started: ${data.action}...`)
      commit('SET_PROGRESS_PERCENTAGE', data.progress_percentage)
    },
    ...
    async onCalibrationFinished({ commit }, data) {
      commit('SET_PROGRESS_MESSAGE', `Calibration process finished: ${data.action}...`)
      setTimeout(() => {
    ...
    async onCalibrationFailed({ commit }, data) {
      commit('SET_PROGRESS_MESSAGE', `Calibration process ${data.action} failed: ${data.error}`)
