...
export default {
  ...
  actions: {
    // data loading
    async load({ dispatch }, { sampleId, ionId, collectionId, filterParams = null }) {
      await dispatch('unload')
      await dispatch('setFilterParams', filterParams ? filterParams : paramDefaults)
    ...
    async loadMatches({ commit, dispatch, state }, params = {}) {
      ...
      const collectionId = params?.collectionId || state.activeIon.target_collection_id
    ...
    async resetVisualization({ commit }) {
      if (!state.tracesSignalTimeseries && !state.tracesSignalSumSpectrum) return
    ...
    async setFilterParams({ commit }, params = null) {
      ...
      const filterParams =
        params || state?.activeIon?.filter_params?.[state.activeIon.instrument] || {}

      // Use instrument-specific filter params or fallback to defaults
      for (const param in paramDefaults) {
        commit(`SET_PARAM_${param.toUpperCase()}`, filterParams[param] ?? paramDefaults[param])
      }
    },
    ...
    async getSampleIonData({ rootGetters, state, dispatch }, { sampleId, ionId, collectionId }) {
