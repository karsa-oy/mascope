...
export default {
  ...
  actions: {
    ...
    async update({ rootState }, sample) {
      await rootState.api.httpClient.updateSampleItem(sample.sample_item_id, sample)
    },
    ...
    async onSampleBatchExportPeaksFailed({ dispatch }, error) {
      await dispatch('app/pushNotification', { message: error, key: Math.random() }, { root: true })
    },
    ...
    // selection
    async updateCollectionSelection({ commit, state }, { collectionId, selectionValue }) {
