...
export default {
  ...
  actions: {
    ...
    async loadBatchSamplesData({ commit, dispatch }, batchId) {
      ...
      commit('SET_MATCH_COMPOUNDS', batchData.batch_matches_info?.match_compounds)
    ...
    async reload({ getters, rootState, state, dispatch, commit }, batch = null) {
    ...
    async batchToggle({ rootState, state, dispatch }, batch) {
      ...
      const active_batch_id = state.active ? state.active.sample_batch_id : null
    ...
    async targetCollectionToggle(
      ...
      state.targetCollections
        .filter(
          (row) => row.target_collection_id != targetCollectionToggledId && row.selection == 2,
        )
  ...
  getters: {
    ...
    targetIon: (state, getters) => (targetIonId) => {
      const [targetIon] = getters['targetIons'].filter((row) => row.target_ion_id == targetIonId)
    ...
    sampleItemsSelected: (state, getters) => {
      return getters['sampleItems'].filter((sampleItem) => sampleItem.selection >= 2)
    },
    sampleItemFocused: (state, getters) => {
      const sampleItem = getters['sampleItems'].filter((sampleItem) => sampleItem.selection == 3)
