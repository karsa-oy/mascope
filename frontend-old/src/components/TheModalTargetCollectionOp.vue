...
export default {
  ...
  computed: {
      ...
      switch (this.action) {
        case 'create':
          return (
            !this.collectionName ||
            !this.collectionType ||
            (this.targetCompounds.length === 0 && this.targetCompoundsCreate.length === 0)
          )

        case 'update':
          // Check if basic properties have changed
          const basicPropertiesChanged =
            this.collectionName !== this.targetCollectionActive.target_collection_name ||
            this.collectionDesc !== this.targetCollectionActive.target_collection_description ||
            this.collectionType !== this.targetCollectionActive.target_collection_type

          // Compare initial and current compounds
          const compoundsChanged =
            !_.isEqual(
              this.initialCompounds.map((compound) => compound.target_compound_id).sort(),
              this.targetCompounds.map((compound) => compound.target_compound_id).sort(),
    ...
    modalTitle() {
      ...
      switch (this.action) {
        case 'create':
          title = `Create a new target collection ${this?.collectionName || ''}`
          break
        case 'update':
    ...
    selectedCompoundsLabel() {
      const name = this.collectionName === '' ? 'new' : `"${this.collectionName}"`
      if (
    ...
    addCompoundsListLabel() {
      ...
      if (this.addCompoundsSource === 'all') return 'Select compounds to add from all compounds:'
    ...
    spreadsheetLabel() {
      if (this.addCompoundsSource === 'spreadsheet' && this.spreadsheetCompounds.length)
    ...
    paginatedInitialCompounds() {
      const start = (this.initialCompoundsCurrentPage - 1) * this.compoundsPerPage
      const end = start + this.compoundsPerPage
      return this.initialCompounds.slice(start, end)
    },
    paginatedAddedCompounds() {
      const start = (this.addedCompoundsCurrentPage - 1) * this.compoundsPerPage
      const end = start + this.compoundsPerPage
      return this.addedCompounds.slice(start, end)
    },
    paginatedTargetCompoundsCreate() {
      const start = (this.targetCompoundsCreateCurrentPage - 1) * this.compoundsPerPage
      const end = start + this.compoundsPerPage
    ...
    filteredCollections() {
      if (this.action !== 'create') {
        return this.allCollections.filter((collection) => {
          return (
            collection.target_collection_id !== this.targetCollectionActive.target_collection_id
          )
        })
      }
    ...
    targetCompoundsIds() {
      return this.targetCompounds.map((compound) => compound.target_compound_id)
    },
    sampleBatchesIds() {
    ...
    newCollection() {
      ...
      if (this.actionIs('update')) {
        return {
          target_collection_id: this.targetCollectionActive.target_collection_id,
          target_collection_name: this.collectionName,
          target_collection_description: this.collectionDesc,
          target_collection_type: this.collectionType,
          target_compound_ids: this.targetCompoundsIds,
  ...
  methods: {
    ...
    initData() {
      ...
      if (this.action == 'create') {
        ...
        if (this.activeWorkspaceBatches) this.reconcileBatches(this.activeWorkspaceBatches)
      }
      ...
      if (this.actionIs('update')) {
        ...
        this.collectionId = this?.targetCollectionActive?.target_collection_id || ''
        this.collectionName = this?.targetCollectionActive?.target_collection_name || ''
        this.collectionDesc = this?.targetCollectionActive?.target_collection_description || ''
        this.collectionType = this?.targetCollectionActive?.target_collection_type || null
        this.targetCompounds = this?.targetCollectionActive?.target_compounds || []
        this.initialCompounds = this.targetCompounds
        this.targetCompoundsCreate = []
        this.spreadsheetCompounds = []
      }
    ...
    reconcileCompounds(compounds) {
      ...
      const combinedCompounds = [...this.initialCompounds, ...this.targetCompounds]
    ...
    async loadSpreadsheetCompounds(rows) {
      ...
      const { existingCompounds, notExistingCompounds } = await this.processSpreadsheetInput(rows)
