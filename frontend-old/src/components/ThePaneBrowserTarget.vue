...
export default {
  ...
  computed: {
    ...
    targetIsotopeRows: function () {
      ...
      const matchIsotopeIds = new Set(this.matchIsotopes.map((mi) => mi.target_isotope_id))
    ...
    targetLevels: function () {
      ...
      return [
        ...
        },
        {
          ...
          detailsOpen: this.sampleItemFocused ? this.matchScoreTagClicked : null,
          rowClick: doNothing,
        },
      ]
    },
    menu() {
      ...
      if (this.targetCollectionsSelected.length == 0 && this.sampleBatchesSelected.length == 1) {
  ...
  methods: {
    ...
    async ionShow({ ionId, collectionId }) {
      ...
      const filterParams =
        this.matchIons.filter((ion) => ion.target_ion_id === ionId)[0]?.filter_params[
          this.sampleItemFocused.instrument
        ] || null
    ...
    matchScoreTagClicked(row) {
      const ionId =
        ...
        this?.matchIons.filter((ion) => ion.target_compound_id === row.target_compound_id)[0]
          ?.target_ion_id ??
