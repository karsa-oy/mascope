  ...
  computed: {
    ...
    progressTitle() {
      const actionTypeCapitalized =
        this.progressActionType.charAt(0).toUpperCase() + this.progressActionType.slice(1)
      ...
      switch (this.progressAction) {
        ...
        default:
          return `${
            this.progressAction.charAt(0).toUpperCase() + this.progressAction.slice(1)
          } Progress`
      }
    },
    progressType() {
    ...
    progressStateValue() {
      return this.progressStateName ? this.$store.state.notification[this.progressStateName] : false
    },
  },
  methods: {
    ...mapMutations({
