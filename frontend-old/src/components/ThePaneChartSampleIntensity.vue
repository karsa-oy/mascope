...
export default {
  ...
  computed: {
    ...
    data: function () {
      ...
      let allCompoundIds = this.targetCompounds.map((compound) => compound.target_compound_id)
      ...
      for (let targetCompoundId of allCompoundIds) {
        ...
        for (let sampleItemId of x) {
          let itemMatches = this.matchCompounds.filter((row) => row.sample_item_id === sampleItemId)
