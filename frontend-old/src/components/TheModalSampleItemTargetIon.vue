          ...
          <div>
            ...
            <div v-if="displayChecklist">
              ...
              <table class="isotopes-table">
                ...
                <tbody>
                  <tr v-for="isotope in activeIsotopes" :key="isotope.target_isotope_id">
                        ...
                        <ul>
                          <li v-for="filter in getFailedFilters(isotope)" :key="filter.filter">
                            ...
                            <span v-if="filter.threshold !== 'N/A'"
                              >({{ filter.filter }} is {{ filter.threshold }})</span
                            >
                          </li>
                        </ul>
                      </div>
              ...
              <!-- Checklist -->
              <label class="label"
                >1) Is there a clear peak in the signal corresponding target isotope?</label
              >
              <div v-for="isotope in activeIsotopes" :key="isotope.target_isotope_id">
                ...
                <input
                  ...
                  @input="
                    (event) => (checklist.isotopeRating[isotope.mz] = Number(event.target.value))
                  "
              ...
              <label class="label"
                >2) Do the timeseries indicate a good match between the isotopes?</label
              >
              ...
              <label class="label"
                >3) Do the timeseries indicate expected behavior of the target in question?</label
              >
              ...
              <b-radio v-model="checklist.timeseriesExpectedBehavior" native-value="1"
                >Do not know</b-radio
              >
...
export default {
  ...
  computed: {
    ...
    modalTitle() {
      ...
      if (this.activeIon) {
        const ionSumIntensity = this.formatNumber(this.activeIon.sample_peak_area_sum)
  ...
  methods: {
    ...
    getFailedFilters(isotope) {
      ...
      if (Math.abs(isotope.match_abundance_error) > this.paramIsotopeRatioTolerance) {
        failedFilters.push({
      ...
      if (Math.max(isotope.match_isotope_correlation, 0) < this.paramMinIsotopeCorrelation) {
        failedFilters.push({
    ...
    async submitRating() {
      ...
      if (this.displayChecklist) {
        payload.checklist = {
          ...
          timeseries_expected_behavior: Number(this.checklist.timeseriesExpectedBehavior),
