...
const action = computed(() => {
  return modalStore.targetCollectionOpProps.action
})
...
function actionIs(...actions) {
  return actions.includes(modalStore.targetCollectionOpProps.action)
}
...
function resetData() {
  modalStore.targetCollectionOpProps = {}
  activeTab.value = 'info'
  compoundsTab.value = 'selectedCompounds'
  collectionId.value = ''
  collectionName.value = ''
...
<template>
  <section>
    <b-modal
      v-model:active="modalStore.targetCollectionOpActive"
      has-modal-card
      trap-focus
      :can-cancel="true"
      aria-role="dialog"
