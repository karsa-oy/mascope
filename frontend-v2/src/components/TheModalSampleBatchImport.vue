  ...
  methods: {
    ...
    processButtonClick() {
      this.$buefy.dialog.confirm({
        ...
        } samples into the batch '${this.batchActive?.sample_batch_name || ''}'?`,
        confirmText: 'Import',
        type: 'is-primary',
        hasIcon: true,
        icon: 'file-import',
    ...
    prepareSampleItemsToCreate() {
      if (this.importType === 'autosampler') {
        ...
        for (let [i, row] of Object.entries(this.parsedRows)) {
          let newSampleItem = {
            filename: this.sampleBatchImportProps.sampleFilesSelected[i]?.filename || null,
      ...
      if (this.importType === 'general') {
        ...
        this.sampleItemsToCreate = this.parsedRows.map((row, index) => {
          const newSampleItem = {
            sample_batch_id: this.batchActive.sample_batch_id,
            filename: this.sampleBatchImportProps.sampleFilesSelected[index]?.filename || null,
    ...
    validateColumns(cols) {
      ...
      if (this.importType === 'autosampler') return (this.columnsValidation = true)
    ...
    validateImportedSampleItems() {
      ...
      if (
        this.sampleItemsToCreate.length !== this.sampleBatchImportProps.sampleFilesSelected.length
      ) {
