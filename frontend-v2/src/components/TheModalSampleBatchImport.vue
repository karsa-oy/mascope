<script setup>
  ...
  const sampleItemsToCreateLabel = computed(() => importType.value == 'autosampler'
  ...
  function processButtonClick() {
    dialog.confirm({
      message: `Are you sure you want to import ${sampleItemsToCreate.value.length
  ...
  function prepareSampleItemsToCreate() {
    if (importType.value === 'autosampler') {
      ...
      for (let [i, row] of Object.entries(parsedRows)) {
        let newSampleItem = {
          filename: modalStore.sampleBatchImportProps
            .sampleFilesSelected[i]?.filename ?? null,
    ...
    if (importType.value === 'general') {
      ...
      sampleItemsToCreate.value = parsedRows.value.map((row, index) => {
        const newSampleItem = {
          sample_batch_id: batchStore.active.sample_batch_id,
          filename: modalStore.sampleBatchImportProps
            .sampleFilesSelected[index]?.filename ?? null,
  ...
  function validateColumns(cols) {
    ...
    if (
      failedValidations.messages.length > 0 ||
      failedValidations.columnsFailures.length > 0
    ) {
  ...
  function validateImportedSampleItems() {
    ...
    if (
      sampleItemsToCreate.value.length !== modalStore.sampleBatchImportProps.sampleFilesSelected.length
    ) {
      failedValidations.messages.push(
        `The number of pasted samples (${sampleItemsToCreate.value.length}) doesn't line up with the total number of selected files (${modalStore.sampleBatchImportProps.sampleFilesSelected.length}).
    ...
    if (
      failedValidations.messages.length > 0 ||
      failedValidations.sampleFailures.length > 0
    ) {
      notificationStore.showWarningNotification({
        notification: 'validationErrors',
        data: failedValidations,
...
<template>
  <section>
    <b-modal v-model:active="modalActive" trap-focus :can-cancel="true" aria-role="dialog" aria-modal
      ...
      <div class="modal-card" style="height: 95vh; width: auto">
        ...
        <section class="modal-card-body" style="min-height: 250px">
          <b-tabs v-model="activeTab" type="is-boxed" position="is-centered" expanded>
            ...
            <b-tab-item value="input" label="Data Input">
              <base-spreadsheet-input label="CSV" :cols="csvCols" :colsFromHeader="true" @colsPasted="processCsvCols"
                ...
                <b-field label="Please select the Filter ID">
                  <div style="display: flex; justify-content: space-between; align-items: center">
                    <div style="display: flex; align-items: center">
                      <b-button type="is-primary" icon-left="plus" @click="generateFilterId" style="margin-right: 10px">
                      </b-button>
                      <b-input v-model="selectedFilterId" disabled expanded></b-input>
                      <b-dropdown aria-role="list" v-model="selectedFilterId" expanded>
                        <template #trigger>
                          <b-button :label="selectedFilterId || 'Select Filter ID'" icon-right="menu-down" />
                        </template>
                    ...
                    <b-button type="is-primary" @click="preprocessSamples" :disabled="!selectedFilterId">
                      Continue
                    </b-button>
                  </div>
                </b-field>
            ...
            <b-tab-item value="samples" label="Sample Items" :disabled="sampleItemsToCreate.length == 0">
              <b-field :label="sampleItemsToCreateLabel" v-if="sampleItemsToCreate.length > 0">
                <div class="table-with-pagination">
                  <div class="table-container">
                    <b-table v-if="sampleItemsToCreate.length > 0" :data="paginatedSampleItemsToCreate"
                      :columns="tableColumns"></b-table>
                  </div>
                  <div class="pagination-container">
                    <b-pagination :total="sampleItemsToCreate.length" v-model:current="samplesCurrentPage"
                      :per-page="samplesPerPage" size="is-small"></b-pagination>
                  </div>
                </div>
              </b-field>
            </b-tab-item>
        ...
        <footer class="modal-card-foot">
          ...
          <b-button expanded type="is-primary" :disabled="!sampleItemsValidation" @click="processButtonClick">
