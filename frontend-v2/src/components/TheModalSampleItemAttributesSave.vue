<script setup>
  ...
  import { dialog, toast } from '@ntohq/buefy-next'
  ...
  formFields.value = structuredClone(defaultTemplate.value.template)

  const availableTemplates = computed(() => ([defaultTemplate.value, ...savedTemplates.value]))
  const batchFilterIds = computed(() => batchStore.batchActive
  ...
  const instrumentIsTof = computed(() => sampleInstrument.value
    ? sampleInstrument.value.indexOf('ORBI') == -1
    : false
  )
  const mzCalibrationTableRows = computed(() => (calibrationStore.mzFitStats ?? []))
  const sampleItemAttributes = computed(() => formFields.value
  ...
  const sampleItemName = computed(() => formFields.value
    .filter((field) => field.label == 'sample_item_name')[0].value
  )
  const savedTemplates = computed(() => appStore.attributeTemplates.value
  ...
  function deleteTemplate() {
    dialog.confirm({
      ...
      message:
        'Are you sure you want to delete template <b>' + loadedTemplate.value.name + '</b>?',
      confirmText: 'Delete',
      onConfirm: () => {
        const templateToDelete = availableTemplates.value.find(
          (template) =>
            template.attribute_template_id == loadedTemplate.value.attribute_template_id,
        )
  ...
  watch(modalStore.sampleItemAttributesSaveActive, (newValue) => {
    if (newValue) {
      activeStep.value = 0
    } else {
  ...
  watch(modalStore.sampleItemAttributesSaveProps, (data) => {
...
<template>
  <section>
    <b-modal v-model:active="modalStore.sampleItemAttributesSaveActive" trap-focus :can-cancel="true" aria-role="dialog"
      aria-modal @close="modalStore.deactivate">
        ...
        <b-steps v-model="activeStep" :has-navigation="false">
          <b-step-item label="Sample information" :clickable="true"
            ...
            <div style="text-align: right" v-if="editable">
              <b-button icon-right="cog" type="is-primary" size="is-small"
              ...
              <template>
                <b-field :label="convertLabelToTitle(item.label)">
                  <b-input v-model="item.value"
                    ...
                    :required="fillable && item.required" :disabled="!fillable || item.disabled" lazy expanded>
                  </b-input>
                  <div v-if="showEditFunctions">
                    <b-button :id="item.label" :disabled="item.required" @click="removeField" type="is-danger"
                      icon-right="delete" hover title="Delete Field">
                    </b-button>
                  </div>
                </b-field>
              </template>
            ...
            <div>
              <b-field label="Filter ID">
                ...
                <b-dropdown aria-role="list" v-model="sampleItemFilterId" expanded>
                  <template #trigger>
                    <b-button :label="sampleItemFilterId" icon-right="menu-down" style="align: left" />
                  </template>
              ...
              <b-field label="Sample type">
                <b-dropdown aria-role="list" v-model="sampleItemType" expanded>
                  <template #trigger>
                    <b-button :label="sampleItemType" icon-right="menu-down" expanded style="align: left" />
                  </template>
                  <b-dropdown-item aria-role="listitem" value="INSTRUMENT_BACKGROUND"
                    v-if="!sampleItemFilterId">Instrument
                    background</b-dropdown-item>
                  <b-dropdown-item aria-role="listitem" value="FILTER_REGENERATION" v-if="sampleItemFilterId">
                    Filter regeneration
                  </b-dropdown-item>
                  <b-dropdown-item aria-role="listitem" value="FILTER_BACKGROUND" v-if="sampleItemFilterId">
                    Filter background
                  </b-dropdown-item>
                  <b-dropdown-item aria-role="listitem" value="SAMPLE"
                    v-if="sampleItemFilterId">Sample</b-dropdown-item>
                  <b-dropdown-item aria-role="listitem" value="BLANK" v-if="sampleItemFilterId">Blank</b-dropdown-item>
                  <b-dropdown-item aria-role="listitem" value="UNKNOWN"
                    v-if="sampleItemFilterId">Unknown</b-dropdown-item>
                </b-dropdown>
              </b-field>
                ...
                <div class="row">
                  <div class="columns">
                    ...
                    <div class="column is-narrow" style="text-align: left" v-if="showEditFunctions">
                      <b-button :disabled="!loadedTemplate || !loadedTemplate.name || loadedTemplate.name == 'default'"
                        @click="deleteTemplate" type="is-danger" icon-right="delete" hover title="Delete Template">
                      </b-button>
                    </div>
                    <div class="column is-narrow" style="text-align: left" v-if="showEditFunctions">
                      <b-button @click="saveTemplate" :disabled="!formFields.length" type="is-success"
                        icon-left="content-save" hover title="Save Template">
                      </b-button>
                    </div>
                    <div class="column is-one-half" style="text-align: right">
                      <b-button :disabled="!sampleItemType ||
      formFields.filter((f) => f.required).length !=
      formFields.filter((f) => f.required).filter((f) => f.value).length ||
      (action == 'create' && sampleActive)
      " type="is-success" icon-left="content-save" @click="saveSampleItem">
                        Save sample info
                      </b-button>
                    </div>
                  </div>
          ...
          <b-step-item label="Calibration" :visible="instrumentIsTof" :clickable="sampleStore.active ? true : false"
            ...
            <b-collapse :open="false" animation="slide">
              <template #trigger>
                <section style="padding: 0.5em">
                  <b-button icon-left="wrench" size="is-small" @click="(props) => { props.open = !props.open }">
                  </b-button>
                </section>
              </template>
            ...
            <base-table :key="mzCalibrationTableKey" :rows="mzCalibrationTableRows" :cols="mzCalibrationTableCols"
              :checkable="false" :defaultSort="['mz', 'asc']" :searchable="false" :minPrecision="4" :maxPrecision="4">
            </base-table>
            <div style="text-align: right">
              <b-button :disabled="sampleStore.active ? false : true" type="is-primary" icon-left=""
                @click="mzCalibrationFit">
                Fit
              </b-button>
              <b-button :disabled="calibrationStore.mzFit ? false : true" type="is-success" icon-left="content-save"
          ...
          <b-step-item label="Target search" :clickable="sampleStore.active
      ? !instrumentIsTof || sampleMzCalibrated
        ? true
        : false
      : false
      " :type="{ 'is-success': sampleMatched }">
            ...
            <div style="text-align: center">
              <b-button :disabled="sampleStore.active ? false : true" type="is-success" icon-left=""
                @click="sampleMatch" v-if="!sampleMatched">
                Process
              </b-button>
              <b-button type="is-primary" icon-left="close" @click="modalStore.deactivate" v-if="sampleMatched">
                Close
              </b-button>
            </div>
          </b-step-item>
