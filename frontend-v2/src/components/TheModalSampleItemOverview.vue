<script setup>
  ...
  const action = computed(() => {
    return modalStore.sampleItemOverviewProps.action
  })
  const sampleItem = computed(() => {
    return modalStore.sampleItemOverviewProps && modalStore.sampleItemOverviewProps.sample
      ? modalStore.sampleItemOverviewProps.sample
      : sampleStore.active
        ? sampleStore.active
        : null
  })
  ...
  function initData() {
    if (modalStore.sampleItemOverviewProps.action === 'copy') {
  ...
  async function loadWorkspaceData() {
    ...
    const workspaceBatches = await workspaceStore.getWorkspaceBatches(workspaceSelected.value.workspace_id)
  ...
  // watchers
  watch(modalStore.sampleItemOverviewActive, (newVal) => {
    if (newVal) {
      initData()
    }
  }, { immediate: true })
...
<template>
  <section>
    <b-modal v-model:active="modalStore.sampleItemOverviewActive" trap-focus :can-cancel="true" aria-role="dialog"
      aria-modal @after-enter="initData" @close="modalStore.deactivate">
      <template v-if="actionIs('copy')">
        <div class="modal-card" style="width: 500px">
          ...
          <section class="modal-card-body" style="min-height: 250px">
            ...
            <!-- Batch Selection -->
            <b-field label="Select a batch to copy the item to:" v-if="sameBatch || batches.length > 0">
          ...
          <footer class="modal-card-foot">
            ...
            <b-button type="is-primary" icon-left="content-save" expanded :loading="isCopying"
              :disabled="!newItemName || !workspaceSelected || !batchSelected || isCopying" @click="copySampleItem">
