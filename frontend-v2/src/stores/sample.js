...
export const useSampleStore = defineStore('sample', () => {
  ...
  const alarmCategory = computed(() => {
    ...
    const alarmCompoundIds = targetCompounds
      .filter((compound) => alarmCollectionIds
        .includes(compound.target_collection_id)
      )
      .map((compound) => compound.target_compound_id)
    // Get match compound data for alarming compounds
    const alarmCompoundMatches = matchCompounds.value
      .filter((match) =>
  ...
  async function loadSampleData(sampleItemId) {
    ...
    matchCollections.value = sampleData.match_collections
        ?.map((coll) => {
          if (
            activeCollection &&
