...
export const useWorkspaceStore = defineStore('workspace', () => {
    ...
    const sampleBatch = computed(() => (sampleBatchId) => batches.value
      .find((batch) => batch.sample_batch_id == sampleBatchId)
    )
    const sampleBatches = computed(() => batches.value ?? [])
    const sampleBatchesSelected = computed(() => sampleBatches.value
      .filter((sampleBatch) => sampleBatch.selection >= 2)
    )
