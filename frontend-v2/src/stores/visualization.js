...
export const useVisualizationStore = defineStore('visualization', () => {
  ...
  const defaultFilterParams = computed(() => {
    ...
    const transformedParamDefaults = Object.entries(paramDefaults)
      .reduce((acc, [key, value]) => {
        // Prepend 'param' and convert to camelCase
        const camelCaseKey = 'param' + snakeToCamel(
  ...
  async function loadVisualizationIonFocus(params = {}) {
    ...
    if (tracesSignalTimeseries.value && tracesSignalSumSpectrum.value)
  ...
  async function setFilterParams(params = null) {
    ...
    const filterParams = params 
      ?? activeIon.value?.filter_params?.[activeIon.value.instrument]
      ?? {}

    // Use instrument-specific filter params or fallback to defaults
    paramMzTolerance.value = filterParams.mz_tolerance
      ?? paramDefaults.mz_tolerance
    paramMinIsotopeAbundance.value = filterParams.min_isotope_abundance
    ?? paramDefaults.min_isotope_abundance
    paramIsotopeRatioTolerance.value = filterParams.isotope_ratio_tolerance
    ?? paramDefaults.isotope_ratio_tolerance
    paramPeakMinIntensity.value = filterParams.peak_min_intensity
    ?? paramDefaults.peak_min_intensity
    paramMinIsotopeCorrelation.value = filterParams.min_isotope_correlation
    ?? paramDefaults.min_isotope_correlation
    paramProbableMatchThreshold.value = filterParams.probable_match_threshold
    ?? paramDefaults.probable_match_threshold
    paramPossibleMatchThreshold.value = filterParams.possible_match_threshold
    ?? paramDefaults.possible_match_threshold
  }
