...
async def get_ms2_summary(
    ...
    summary = await m_compute.get_ms2_summary(
        sample.filename,
        t_min=sample.t0,
        t_max=sample.t1,
...
async def get_ms2_averaged_centroids(
    ...
    ms2_by_parent = await m_compute.get_orbi_ms2_centroids_by_parent(
        sample.filename,
        t_min=sample.t0,
        t_max=sample.t1,
        polarity=sample.polarity,
...
async def get_ms2_timeseries(
    ...
    timeseries = await m_compute.get_ms2_fragment_timeseries(
        sample.filename,
        parent_peak_mz,
        t_min=sample.t0,
        t_max=sample.t1,
