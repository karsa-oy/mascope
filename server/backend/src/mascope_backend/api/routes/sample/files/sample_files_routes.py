...
# the legacy POST /upload route; the web app authenticates with its
# cookie either way. Without the stamp, get_enabled_backends rejects
# every Bearer-token request to these routes with 401.
for _route in sample_files_upload_router.routes:
