"""
Schema configuration for v41 database tables. Each key in the dictionary is a table name,
and its value is a dictionary specifying details about the table's schema and configuration.

Key changes from v40:
- Added sample_item.sample_file_id (FK to sample_file.sample_file_id)
- Removed sample_item.filename column
- Added index on sample_item.sample_file_id for join performance
...
table_configs = {
    ...
    "match_isotope": {
        "columns": {
            ...
            "match_isotope_similarity": ("FLOAT", 1, None, 0),
            "match_score": ("FLOAT", 1, None, 0),
