"""
Maintenance script to backfill the 'sparsity' variable in peak_timeseries.zarr
for all existing sample files.

For each sample file in the database, checks if 'sparsity' exists in its
...
Usage:
    mascope dev db script run populate_sparsity
    mascope prod db script run populate_sparsity
...
async def populate_sparsity() -> dict:
    """Backfill 'sparsity' for all sample files where it is missing.
    ...
    total = len(filenames)
    runtime.logger.info(f"Found {total} sample file(s) to check for 'sparsity'")
    ...
    for i, filename in enumerate(filenames, 1):
        try:
            was_created = m_io.ensure_sparsity_exists(filename)
            if was_created:
                migrated += 1
            else:
                skipped += 1
...
async def run() -> None:
    ...
    result = await populate_sparsity()

    runtime.logger.info("=" * 80)
    runtime.logger.info("POPULATE sparsity COMPLETE")
    runtime.logger.info(f"Total sample files checked : {result['total']}")
    runtime.logger.info(f"Migrated (sparsity added) : {result['migrated']}")
