...
export const useAcquisition = defineStore("app.data.acquisition", () => {
  ...
  const orbi = computed(() =>
    instrument.focused.instrument.toLowerCase().includes("orbi"),
  );
  // instrument
  ...
  async function loadMzCalibration() {
    const lastMzCalibration = await api.http.get(
      `/calibration/mz_calibration`,
      {
        params: {
          instrument: instrument.focused?.instrument,
        },
