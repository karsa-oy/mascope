...
const invalid = computed(() => {
  const invalidOrbi =
    processed.value.invalid.orbi.length > 0 &&
    instrumentType(instrument.orbi) !== "orbi";
  const invalidTof =
    processed.value.invalid.tof.length > 0 &&
    instrumentType(instrument.tof) !== "tof";
  const invalidInstrumentName =
    !validInstrumentName(instrument.tof) ||
    !validInstrumentName(instrument.orbi);
  const noFiles =
    processed.value.invalid.tof.length +
      processed.value.invalid.orbi.length +
...
const upload = () => {
  const renamedTofFiles = processed.value.invalid.tof.map(
    (file) =>
      new File([file], `${instrument.tof}_${file.name}`, { type: file.type }),
  );
  const renamedOrbiFiles = processed.value.invalid.orbi.map(
    (file) =>
      new File([file], `${instrument.orbi}_${file.name}`, { type: file.type }),
  );
  active.value = false;
  app.data.sample.upload([
    ...renamedTofFiles,
    ...renamedOrbiFiles,
    ...processed.value.valid,
  ]);
};
</script>
...
<template>
  <Dialog :visible="active" modal header="Resolve issues with uploaded files">
    ...
    <template v-if="processed.invalid.tof.length > 0">
      ...
      <p>
        The following h5 files do not have a valid TOF instrument name as a
        prefix:
      </p>
      <ul>
      ...
      <p>
        <i>
          Valid TOF instrument names must include the word TOF or API (in lower
          or upper case) and consist only of letters, numbers and underscores _.
        </i>
      </p>
      ...
      <div class="center" style="width: 100%">
        ...
        >
          <i>
            You entered an invalid TOF instrument name; please enter a valid
            instrument name, as explained above.
          </i>
        </Message>
      </div>
    </template>
    ...
    <template v-if="processed.invalid.orbi.length > 0">
      ...
      <p>
        The following h5 files do not have a valid OrbiTrap instrument name as a
        prefix:
      </p>
      <ul>
      ...
      <p>
        <i>
          Valid OrbiTrap instrument names must include the word ORBI (in lower
          or upper case) and consist only of letters, numbers and underscores _.
        </i>
      </p>
      <p>Please select or enter an instrument to assign these files to:</p>
      <div class="center" style="width: 100%">
        ...
        >
          <i>
            You entered an invalid OrbiTrap instrument name; please enter a
            valid instrument name, as explained above.
          </i>
        </Message>
      </div>
    </template>
