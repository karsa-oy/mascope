...
export const chartMixin = {
    ...
    data: function () {
        return {
            name: "ThePageAnalysisChart_" + this.id,
    ...
    methods: {
        ...
        figure_cache_add_ref(zoom_stack_item) {
            ...
            this.figure_cache[zoom_stack_item.id] = {
                ...
                'figure_layout': _.clone(this.figure_layout_default),
                'figure_traces': [],
            };
            return this.figure_cache[zoom_stack_item.id];
        },
        ...
        init_figure() {
            ...
            if (fs.existsSync('configs/figure_config.json')) {
                ...
                self.figure_config = _.clone(figure_configs.common_config);
                self.figure_layout_default = _.clone(figure_configs[self.id].layout);
                self.figure_layout = _.clone(self.figure_layout_default);
            ...
            figure_div.on("plotly_relayout", function (eventData) {
                ...
                if (Object.keys(eventData).length) {
                    // zoom_in
                    let prev_ranges = _.clone(self.zoom_stack.slice(-1)[0]);
            ...
            figure_div.on('plotly_doubleclick', function () {
                // Signal double click to all ThePageAnalysisCharts
        ...
        on_plotly_click(eventData) {
            // Plotly click event handler, override in ThePageAnalysisChart component
            // this.log(eventData);
        ...
        async _on_figure_data(json_data) {
            ...
            if (_.isEmpty(json_data)) {
                // reset the figure
                self.figure_layout = _.clone(self.figure_layout_default);
                self.figure_traces = _.clone(self.figure_traces_default);
            ...
            if (_.isEqual(zoom_stack_item_id,
                ...
                await self.update_figure(_.clone(self.zoom_stack.slice(-1)[0]));
            }
        },
        ...
        reset_figure() {
            this.figure_layout = _.clone(this.figure_layout_default);
            this.figure_traces = _.clone(self.figure_traces_default);
            this.peak_traces = [];
        },
        ...
        reset_figure_cache() {
            ...
            for (let i = 0; i < this.zoom_stack.length; ++i) {
                cancel_requests.push(_.clone(this.zoom_stack[i].id));
            }
        ...
        async update_figure(zoom_stack_item = null) {
            ...
            if (!_.isNull(zoom_stack_item)) {
                ...
                self.figure_layout = {
                    ...
                    'annotations': _.clone(this.figure_annotations)
                };
                self.figure_traces = [
        ...
        visualize_range_on_sample_selected(new_ranges) {
            ...
            // Update figure
            let cur_ranges = _.clone(this.zoom_stack.slice(-1)[0]);
        ...
        visualize_range_on_zoom_in(prev_ranges, new_ranges) {
            ...
            if (zoom_stack_item) {
                ...
                zoom_stack_item = _.clone(zoom_stack_item);
            } else {
                // Create new zoom_stack_item
            ...
            // Update figure
            let cur_ranges = _.clone(self.zoom_stack.slice(-1)[0]);
            // Set new ranges
        ...
        visualize_range_on_zoom_out() {
            ...
            let zoom_stack_item_to_restore = _.clone(self.zoom_stack.slice(-1)[0]);
            // collect client_rooms to cancel for stop_visualize_range call
            let cancel_requests = [];
            cancel_requests.push(_.clone(zoom_stack_item_to_remove.id));
            // Loop until persistent item found from zoom stack
            while (zoom_stack_item_to_restore.volatile) {
                cancel_requests.push(_.clone(zoom_stack_item_to_restore.id));
                // Release reference of popped item
                self.figure_cache_release_ref(zoom_stack_item_to_restore.id);
                // Get next item from stack
                self.zoom_stack.pop();
                zoom_stack_item_to_restore = _.clone(self.zoom_stack.slice(-1)[0]);
            }
