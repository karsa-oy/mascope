        ...
        <div class="modal-card">
          <header class="modal-card-head">
            ...
            <b-button
              ...
              @click="
                isotope_table_show_only_checked =
                  !isotope_table_show_only_checked
              "
            >
            </b-button>
      ...
      >
        <div class="columns">
          <div class="modal-card" style="width: 500px; height: 700px">
            ...
            <footer class="modal-card-foot">
              <b-button @click="is_mz_calib_modal_active = false">
                Cancel
              </b-button>
      ...
      >
        <template slot="detail" slot-scope="props">
          <tr
            ...
            :key="item['target id'] + '/' + item['ion id']"
            @click="() => selectIon(item['target id'], item['ion id'])"
            v-bind:class="[
      ...
      <div class="columns">
        ...
        <div class="column" style="text-align: right">
          <b-button
            ...
            outlined
            @click="
              isotope_table_show_only_checked = !isotope_table_show_only_checked
            "
          >
          </b-button>
          <b-button
...
export default {
  ...
  computed: {
    ...
    target_compound_selected: {
      ...
      set(value) {
        this.$store.commit("target_compound_selected", value);
      },
    },
    target_ion_selected: {
      get() {
        return this.$store.state.target_ion_selected;
      },
      set(value) {
  ...
  created: function () {
    ...
    if (fs.existsSync("configs/figure_config.json")) {
      const figure_configs = JSON.parse(
        fs.readFileSync("configs/figure_config.json", "utf8")
      );
  ...
  methods: {
    ...
    drawMzCalibStatsFigure() {
      ...
      let pre_trace = { x: mz, y: pre_calib_dmz, type: "bar", name: "Pre" };
      let post_trace = { x: mz, y: post_calib_dmz, type: "bar", name: "Post" };
      let d_trace = { x: mz, y: ddmz, type: "line", name: "Diff" };
    ...
    fitMzCalibFunction() {
      ...
      let peak_mzs = this.isotope_table_checked_rows.map(
        (row) => row["peak mz"]
      );
