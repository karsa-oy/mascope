<template>
  <div>
    ...
    <section class="add-log-entry-modal">
      <b-modal
        :active.sync="is_modal_add_log_entry_active"
        has-modal-card
        trap-focus
        :can-cancel="true"
    ...
    <section class="instrument-log-modal">
      <b-modal
        :active.sync="is_modal_instrument_log_active"
        has-modal-card
        trap-focus
        :can-cancel="true"
      ...
      >
        <div class="modal-card" style="width: 500px">
          ...
          <div>
            ...
            <section class="modal-card-body">
              <b-table :data="instrument_log_rows">
          ...
          <footer class="modal-card-foot">
            ...
            <b-button is-dark @click="is_modal_instrument_log_active = false">
              Close
            </b-button>
          </footer>
        </div>
          ...
          <div class="modal-card" style="width: auto">
            ...
            <section class="modal-card-body idparams-edit-body">
              ...
              <b-field label="Cassette in">
                <b-switch v-model="desorption_step_modal_filter_in"> </b-switch>
              </b-field>
            </section>
            <footer class="modal-card-foot">
              <b-button
                ...
                :disabled="
                  !(
                    desorption_step_modal_temperature &&
                    desorption_step_modal_time
                  )
                "
              >
                Save
              </b-button>
              <b-button @click="is_modal_desorption_step_active = false">
                Close
              </b-button>
            </footer>
          </div>
              ...
              <div style="text-align: center">
                <div v-if="method.tofdaq.acquisition_mode === 'manual'">
                  <b-button
                    ...
                    :disabled="
                      method.tofdaq.acquisition_mode == 'triggered' ||
                      instrument_status == 'not_ready'
                    "
...
export default {
  ...
  data: function () {
    return {
      ...
      log_entry_default_template: [{ label: "Log text", value: "" }],
      log_entry_fields: [],
  ...
  methods: {
    addDesorptionCycleStep() {
      ...
      this.method.desorption_cycle = this.method.desorption_cycle.sort(
        function (a, b) {
    ...
    drawDesorptionChart() {
      ...
      for (let i in this.method.desorption_cycle) {
        ...
        if (step.filter_in != filter_in) {
          ...
          let prev_trace =
            this.desorption_chart_data[this.desorption_chart_data.length - 1];
        ...
        this.desorption_chart_data[
          this.desorption_chart_data.length - 1
        ].x.push(step.time);
        this.desorption_chart_data[
          this.desorption_chart_data.length - 1
        ].y.push(step.temperature);
        if (filter_in) {
          this.desorption_chart_data[
            this.desorption_chart_data.length - 1
          ].fill = "tozeroy";
        ...
        desorption_chart_layout["yaxis"].ticktext.push(
    ...
    initializeMethod() {
      try {
        if (fs.existsSync("configs/tofcontrol_config.json")) {
          let tofcontrol_config = JSON.parse(
            fs.readFileSync("configs/tofcontrol_config.json", "utf8")
          );
    ...
    saveMethod() {
      fs.writeFileSync("configs/tofcontrol_config.json", this.method_json);
      this.button_save_method_type = "is-success";
    },
    launchDesorptionStepModal() {
      this.desorption_step_modal_filter_in = this.desorption_table_selected_row
        ? this.desorption_table_selected_row.filter_in
        : true;
      this.desorption_step_modal_time = this.desorption_table_selected_row
        ? this.desorption_table_selected_row.time
        : null;
      this.desorption_step_modal_temperature = this
        .desorption_table_selected_row
        ? this.desorption_table_selected_row.temperature
        : null;
  ...
  watch: {
    ...
    acquisition_started: function (new_value, old_value) {
      ...
      this.new_file = { ...new_value, method: this.method };
    },
    ...
    "namespace.connected": function (new_value) {
      if (new_value) {
        ...
        this.namespace.on("instrument_log", (value) =>
          this.be.import_one_way_binding_prop("instrument_log", value.value)
        );
