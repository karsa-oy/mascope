...
the instrument-function fit reads the summed profile and derives a resolution
function from per-peak FWHM. Here we run the real fit internals
...
def test_resolution_fit_matches_thermo(path):
    ...
    # is inherently a bit noisy; ~10% catches gross regressions (e.g. a wrong
    # averaging grid that broadens or narrows every peak) while tolerating
    # peak-selection differences. Measured ~0.95 on a Q Exactive Plus file.
