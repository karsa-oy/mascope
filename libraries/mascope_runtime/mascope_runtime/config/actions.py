...
def autoload():
    config_path = path(state.config_temp or state.config_active)
    config_name = None
    if config_path:
        with open(config_path, "r") as f:
            config_name=f.read()
    # helpers
    config_file = lambda name: (path(name)
        if (name and os.path.exists(path(name)))
        else None
    )
    # resolve config
    if config_name:
    ...
    for conf in configs:
        if pattern.match(conf):
            with open(os.path.join(config_dir, conf), "rb") as f:
                ...
                default = 'default' if (
                    name == 'prod' or 
                    (name == 'dev' and not 'mascope.prod.toml' in configs)
                ) else None
                status = active or default
                print(name, '*' if (not_active and default) else None, ('*' if active else None))
                selected = (
                    ('*' if (not_active and default) else None)
                    or ('*' if active else None)
                    or ''
                )
                table.add_row(name + selected, config['meta']['description'] or 'n/a', conf, status)
    console = Console()
    console.print(table)