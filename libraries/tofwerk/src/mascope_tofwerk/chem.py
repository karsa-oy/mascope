...
def get_exact_isotope_masses(molComp, abu_limit=0):
    ...
    if (
        TwGetIsotopePattern(
            molComp.encode(), abu_limit, nbrIsotopes, masses, abundances
        )
        == 4
    ):
        # Scale
...
def guess_composition(target_mz, el_filter, tolerance=10e-6):
    ...
    m = np.zeros((1,))
    atomMass = np.array(
        [m[0] for el in elements if TwGetMoleculeMass(el.encode(), m) == 4],
        dtype=np.double,
    )
    ...
    if len(ratio_filter) > 0:
        ...
        filterMaxVal = np.array(
            list(list(zip(*num_filter))[1]) + list(list(zip(*ratio_filter))[3]),
            dtype=np.double,
        )
    else:
        nbrFilters = len(num_filter) + len(ratio_filter)
