...
runtime.logger.info(
    f"Loading Tofwerk TwH5 DLL from: mascope_tofwerk/lib/dlls/{libdir}/{libfile}"
)
h5lib = ct.cdll.LoadLibrary(libpath)
runtime.logger.info("Succesfully Loaded Tofwerk TwH5 DLL")
...
geth5descriptor = (
    h5lib.TwGetH5Descriptor if os.name == "posix" else h5lib._TwGetH5Descriptor
)
...
getsumspectrumfromh5 = (
    h5lib.TwGetSumSpectrumFromH5
    if os.name == "posix"
    else h5lib._TwGetSumSpectrumFromH5
)
...
gettofspectrumfromh5 = (
    h5lib.TwGetTofSpectrumFromH5
    if os.name == "posix"
    else h5lib._TwGetTofSpectrumFromH5
)


def TwGetTofSpectrumFromH5(
    filename,
    spectrum,
    segmentIndex,
    segmentEndIndex,
...
gettofspectrum2fromh5 = (
    h5lib.TwGetTofSpectrum2FromH5
    if os.name == "posix"
    else h5lib._TwGetTofSpectrum2FromH5
)


def TwGetTofSpectrum2FromH5(
    filename,
    spectrum,
    segmentIndex,
    segmentEndIndex,
    ...
    gettofspectrum2fromh5.argtypes = [
        ct.c_char_p,
        _float_array,
        ct.c_int,
        ct.c_int,
...
getstickspectrumfromh5 = (
    h5lib.TwGetStickSpectrumFromH5
    if os.name == "posix"
    else h5lib._TwGetStickSpectrumFromH5
)


def TwGetStickSpectrumFromH5(
    filename,
    spectrum,
    segmentIndex,
    segmentEndIndex,
    ...
    getstickspectrumfromh5.argtypes = [
        ct.c_char_p,
        _float_array,
        ct.c_int,
        ct.c_int,
...
getspecxaxisfromh5 = (
    h5lib.TwGetSpecXaxisFromH5 if os.name == "posix" else h5lib._TwGetSpecXaxisFromH5
)


def TwGetSpecXaxisFromH5(filename, specAxis, axisType, unitLabel, maxMass, writeIndex):
    getspecxaxisfromh5.argtypes = [
        ct.c_char_p,
        _double_array,
        ct.c_int,
        ct.c_char_p,
    ...
    return getspecxaxisfromh5(
        filename, specAxis, axisType, unitLabel, maxMass, writeIndex
    )


getsegmentprofilefromh5 = (
    h5lib.TwGetSegmentProfileFromH5
    if os.name == "posix"
    else h5lib._TwGetSegmentProfileFromH5
)


def TwGetSegmentProfileFromH5(
    filename,
    segmentProfile,
    peakIndex,
    bufStartIndex,
    ...
    getsegmentprofilefromh5.argtypes = [
        ct.c_char_p,
        _float_array,
        ct.c_int,
        ct.c_int,
    ...
    return getsegmentprofilefromh5(
        filename,
        segmentProfile,
        peakIndex,
        bufStartIndex,
...
getbufwriteprofilefromh5 = (
    h5lib.TwGetBufWriteProfileFromH5
    if os.name == "posix"
    else h5lib._TwGetBufWriteProfileFromH5
)


def TwGetBufWriteProfileFromH5(
    filename, profile, peakIndex, segmentStartIndex, segmentEndIndex
):
    getbufwriteprofilefromh5.argtypes = [
        ct.c_char_p,
        _float_array,
        ct.c_int,
        ct.c_int,
    ...
    return getbufwriteprofilefromh5(
        filename, profile, peakIndex, segmentStartIndex, segmentEndIndex
    )


getreguserdatafromh5 = (
    h5lib.TwGetRegUserDataFromH5
    if os.name == "posix"
    else h5lib._TwGetRegUserDataFromH5
)


def TwGetRegUserDataFromH5(
    filename, location, bufIndex, writeIndex, bufLength, dataBuffer, description
):
    if dataBuffer is None:
        getreguserdatafromh5.argtypes = [
            ct.c_char_p,
            ct.c_char_p,
            ct.c_int,
            ct.c_int,
...
def TwGetTimingData(filename, bufOffset, bufCount, writeOffset, writeCount, dataBuffer):
    ...
    return gettimingdata(
        filename, bufOffset, bufCount, writeOffset, writeCount, dataBuffer
    )
...
getintattributefromh5 = (
    h5lib.TwGetIntAttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetIntAttributeFromH5
)


def TwGetIntAttributeFromH5(filename, location, name, attribute):
    getintattributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ndpointer(dtype=np.int32, flags="CONTIGUOUS", shape=1),
    ]
    return getintattributefromh5(filename, location, name, attribute)


getuintattributefromh5 = (
    h5lib.TwGetUintAttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetUintAttributeFromH5
)


def TwGetUintAttributeFromH5(filename, location, name, attribute):
    getuintattributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ndpointer(dtype=np.uint32, flags="CONTIGUOUS", shape=1),
    ]
    return getuintattributefromh5(filename, location, name, attribute)


getint64attributefromh5 = (
    h5lib.TwGetInt64AttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetInt64AttributeFromH5
)


def TwGetInt64AttributeFromH5(filename, location, name, attribute):
    getint64attributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ndpointer(dtype=np.int64, flags="CONTIGUOUS", shape=1),
    ]
    return getint64attributefromh5(filename, location, name, attribute)


getuint64attributefromh5 = (
    h5lib.TwGetUint64AttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetUint64AttributeFromH5
)


def TwGetUint64AttributeFromH5(filename, location, name, attribute):
    getuint64attributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ndpointer(dtype=np.uint64, flags="CONTIGUOUS", shape=1),
    ]
    return getuint64attributefromh5(filename, location, name, attribute)


getfloatattributefromh5 = (
    h5lib.TwGetFloatAttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetFloatAttributeFromH5
)


def TwGetFloatAttributeFromH5(filename, location, name, attribute):
    getfloatattributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ndpointer(dtype=np.float32, flags="CONTIGUOUS", shape=1),
    ]
    return getfloatattributefromh5(filename, location, name, attribute)


getdoubleattributefromh5 = (
    h5lib.TwGetDoubleAttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetDoubleAttributeFromH5
)


def TwGetDoubleAttributeFromH5(filename, location, name, attribute):
    getdoubleattributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ndpointer(dtype=np.double, flags="CONTIGUOUS", shape=1),
    ]
    return getdoubleattributefromh5(filename, location, name, attribute)


getstringattributefromh5 = (
    h5lib.TwGetStringAttributeFromH5
    if os.name == "posix"
    else h5lib._TwGetStringAttributeFromH5
)


def TwGetStringAttributeFromH5(filename, location, name, attribute):
    getstringattributefromh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
...
setintattributeinh5 = (
    h5lib.TwSetIntAttributeInH5 if os.name == "posix" else h5lib._TwSetIntAttributeInH5
)
...
setuintattributeinh5 = (
    h5lib.TwSetUintAttributeInH5
    if os.name == "posix"
    else h5lib._TwSetUintAttributeInH5
)
...
setint64attributeinh5 = (
    h5lib.TwSetInt64AttributeInH5
    if os.name == "posix"
    else h5lib._TwSetInt64AttributeInH5
)
...
setuint64attributeinh5 = (
    h5lib.TwSetUint64AttributeInH5
    if os.name == "posix"
    else h5lib._TwSetUint64AttributeInH5
)


def TwSetUint64AttributeInH5(filename, location, name, attribute):
    setuint64attributeinh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ct.c_uint64,
...
setfloatattributeinh5 = (
    h5lib.TwSetFloatAttributeInH5
    if os.name == "posix"
    else h5lib._TwSetFloatAttributeInH5
)
...
setdoubleattributeinh5 = (
    h5lib.TwSetDoubleAttributeInH5
    if os.name == "posix"
    else h5lib._TwSetDoubleAttributeInH5
)


def TwSetDoubleAttributeInH5(filename, location, name, attribute):
    setdoubleattributeinh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ct.c_double,
...
setstringattributeinh5 = (
    h5lib.TwSetStringAttributeInH5
    if os.name == "posix"
    else h5lib._TwSetStringAttributeInH5
)


def TwSetStringAttributeInH5(filename, location, name, attribute):
    setstringattributeinh5.argtypes = [
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
        ct.c_char_p,
...
changepeaktablefromfile = (
    h5lib.TwChangePeakTableFromFile
    if os.name == "posix"
    else h5lib._TwChangePeakTableFromFile
)


def TwChangePeakTableFromFile(filename, massTable, compressionLevel, callback):
    if callback is None:
        changepeaktablefromfile.argtypes = [
            ct.c_char_p,
            ct.c_char_p,
            ct.c_int,
            ct.c_void_p,
...
getbuftimefromh5 = (
    h5lib.TwGetBufTimeFromH5 if os.name == "posix" else h5lib._TwGetBufTimeFromH5
)


def TwGetBufTimeFromH5(filename, bufTime, bufIndex, writeIndex):
    getbuftimefromh5.argtypes = [
        ct.c_char_p,
        ndpointer(dtype=np.double, shape=1),
        ct.c_int,
        ct.c_int,
    ]
...
h5setmasscalib = (
    h5lib.TwH5SetMassCalib if os.name == "posix" else h5lib._TwH5SetMassCalib
)


def TwH5SetMassCalib(filename, mode, nbrParams, p, nbrPoints, mass, tof, weight):
    h5setmasscalib.argtypes = [
        ct.c_char_p,
        ct.c_int,
        ct.c_int,
        _double_array,
...
getacquisitionlogfromh5 = (
    h5lib.TwGetAcquisitionLogFromH5
    if os.name == "posix"
    else h5lib._TwGetAcquisitionLogFromH5
)


def TwGetAcquisitionLogFromH5(location, index, timestamp, logText):
    getacquisitionlogfromh5.argtypes = [
        ct.c_char_p,
        ct.c_int,
        ndpointer(dtype=np.int64),
        ct.c_char_p,
    ]
    return getacquisitionlogfromh5(location, index, timestamp, logText)


geteventlistdatafromh5 = (
    h5lib.TwGetEventListDataFromH5
    if os.name == "posix"
    else h5lib._TwGetEventListDataFromH5
)


def TwGetEventListDataFromH5(
    filename,
    segStartIndex,
    segEndIndex,
    bufStartIndex,
    ...
    if rv == 4:
        nbrSpectra = (
            (segEndIndex - segStartIndex + 1)
            * (bufEndIndex - bufStartIndex + 1)
        ...
        for i in range(nbrSpectra):
            ...
            while index < nEl:
                ...
                if nSamples > 0:
                    samples.append(
                        np.frombuffer(
                            data[i][index : index + nSamples], dtype=np.float32
                        )
                    )
                    index += nSamples
                else:
...
freeeventlistdata = (
    h5lib.TwFreeEventListData if os.name == "posix" else h5lib._TwFreeEventListData
)
...
getuserdatafromh5 = (
    h5lib.TwGetUserDataFromH5 if os.name == "posix" else h5lib._TwGetUserDataFromH5
)


def TwGetUserDataFromH5(
    filename, location, rowIndex, nbrElements, buf, elementDescription=None
):
    ...
    if buf is None:
        ...
        if type(elementDescription) is list:
            tempdesc = np.zeros(256 * nbrElements[0], dtype=np.uint8)
            ret = getuserdatafromh5(
                filename, location, rowIndex, nbrElements, buf, tempdesc
            )
            elementDescription[:] = bytesToStrings(tempdesc)
            return ret
    return getuserdatafromh5(
        filename, location, rowIndex, nbrElements, buf, elementDescription
    )
