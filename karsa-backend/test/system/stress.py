...
view_samples = ["OrbitrapData_20210122_1028_SRCI_DBrMe__3TCM.raw", "OrbitrapData_20210122_1028_SRCI_DBrMe__4TCM.raw"]
...
def parse_args():
    ...
    parser.add_argument("-vl", "--view_list", help="filename: view list of zarr samples - for stresstest viewing", type=str, default=None)
    parser.add_argument("-sl", "--stream_list", help="filename: stream list of raw samples - for stresstest streaming", type=str, default=None)
    parser.add_argument("-nv", "--n_viewers", help="max number of zarr sample viewers", type=int, default=0)
    parser.add_argument("-td", "--target_data_pool_path", help="target data pool path", type=str, required=True)
    ...
    if any([args.n_viewers, args.view_list]):
        assert all([args.n_viewers, args.view_list]) == True, "for stream testing, stream_list and n_viewers should be both specified"
    ...
    while not shutdown_event.is_set():
        ...
        for fname in stream_samples:
            ...
            asyncio.run(
                ...
                max_exec_time=MAX_STREAMING_TIME)
            )
...
def  run_viewers(n_viewers):
    for _ in range(n_viewers):
        v = Thread(target=viewer_proc)
        v.start()
...
def viewer_proc():
    ...
    print(f"started viewer {current_thread().name}")
    request_ids = []
    for i, fname in enumerate(view_samples):
        if shutdown_event.is_set():
            break
        t_range_max = get_t_range_max_from_zarr_name(fname)
        rq_suffix = client.set_viz_test_params(fname, t_range_max=t_range_max, max_exec_time=MAX_VIEWING_TIME)
        request_id = f'{current_thread().name}:{i}_{rq_suffix}'
        request_ids.append(request_id)
        # TODO: lock viz_request_ids for multi-thread sync?
        viz_request_ids.append(request_id)
        asyncio.run(
    ...
    print(f"stopped viewer {current_thread().name}")
...
def run_until_complete():
    ...
    while True:
        try:
            if args.view_list:      # Run series of visualizations and quit
                for i in range(args.n_viewers):
                    logging.info(f"Running {i+1} viewers: begin " + 10*'<')
                    viz_request_ids = []
                    client.viewed_samples = []
                    run_viewers(n_viewers=i+1)
                    time.sleep(1)
                    ...
                    logging.info(f"Running {i+1} viewers: end " + 10*'>')
                break
...
def run():
    global args, client, shutdown_event, view_samples
